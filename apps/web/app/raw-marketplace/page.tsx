...
export default function RawMarketplacePage() {
  ...
  async function handleCreateListing(e: React.FormEvent) {
    ...
    if (!isAdmin) {
      setCreateError(t('Only the store owner can create listings.', 'ಸ್ಟೋರ್ ಮಾಲಿಕರು ಮಾತ್ರ ಲಿಸ್ಟಿಂಗ್ ರಚಿಸಬಹುದು.'))
      return
    }
          ...
          <main className="flex-1">
            <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 slide-in-up">
              ...
              {isAdmin && (
                ...
                >
                  ...
                  <span>{t('Add Listing', 'ಲಿಸ್ಟಿಂಗ್ ಸೇರಿಸಿ')}</span>
                </button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-20 glass rounded-2xl shadow-xl fade-in">
                ...
                <p className={`mb-6 ${isDark ? 'text-[#bbae9a]' : 'text-[#4a4a4a]'}`}>{t('New listings coming soon. Check back!', 'ಹೊಸ ಲಿಸ್ಟಿಂಗ್‌ಗಳು ಶೀಘ್ರದಲ್ಲಿ ಬರುತ್ತವೆ. ಮತ್ತೆ ನೋಡಿ!')}</p>
                {isAdmin && (
                  <button
